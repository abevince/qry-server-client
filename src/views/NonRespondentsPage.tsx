...
function fetchRespondents(questId: number, schoolId: string): Promise<TUser[]> {
  return fetch(`${API_URL}/respondents/non/${questId}/${schoolId}`)
    .then((res) => res.json())
    .catch((error) => error)
}

const NonRespondentsPage = () => {
  ...
  return (
    <div className="bg-gray-200 min-h-screen w-full flex justify-center">
      <div className="fixed bg-gray-200 w-full pt-4 p-2 md:w-2/3">
        <h1 className="text-xl font-medium text-gray-700">Non-Respondents</h1>
...
export default NonRespondentsPage
